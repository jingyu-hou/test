...
!
      ...
      character*80 pairmasterslave(2,nmat)
!
     ...
     &  nstam,ier,nmpcon(2,*),nmpmat_,phase_inf(4),nflcon,
     &  ipairmat(0:2,*),npair,pair2mat(nmat)
!
     ...
     &  alpha,physcon(*),coefmpcref(*),mpcon(2,nmpmat_,*),
     &  pphase(2,phase_inf(3),7+phase_inf(4),phase_inf(1),*),
     &  cphase(13+phase_inf(4),phase_inf(1),*),
     &  phaseother(15+phase_inf(1)*phase_inf(4),*),flcon(0:1,ntmat_,*),
     &  lh(*),enthactrl(*),rdpcon(10,3,*),gscon(30,5,*)
!
      integer nphase(11,*)
!
...
!
     ...
     &        inl,ipoinp,inp,ipoinpc,nelcon,ier,mpcon,nmpcon,nmpmat_)
!
...
!
         elseif(textpart(1)(1:16).eq.'*CREEP-SOFTENING') then
            call creepsoftenings(inpc,textpart,
...
c
     ...
     &        iaxial,ier)
!
         elseif(textpart(1)(1:7).eq.'*DAMAGE') then
            call damages(inpc,textpart,rdpcon,
     &        nmat,ncmat_,iperturb,irstrt,istep,istat,n,
...
!
     ...
     &        iamplitudedefault,namtot,ier)
            dload_flag=.true.
!
         elseif(textpart(1)(1:9).eq.'*DYNAMIC ') then
     ...
     &        ctrl,tincf,nener,ier)
         elseif(textpart(1)(1:25).eq.'*DYNAMICRECRYSTALLIZATION') then
            call dynamicrecrystallizations(inpc,textpart,gscon,
     &        nmat,ncmat_,iperturb,irstrt,istep,istat,n,
     &        iline,ipol,inl,ipoinp,inp,ipoinpc,ier,nstate_)
!
...
!
     ...
     &           ier)
!
         elseif(textpart(1)(1:15).eq.'*LIQUIDFRACTION') then
            call liquidfractions(inpc,textpart,flcon,nflcon,lh,
     &        imat,ntmat_,irstrt,istep,istat,n,iline,ipol,
     &        inl,ipoinp,inp,ipoinpc,ier)
!
...
!
         elseif(textpart(1)(1:12).eq.'*METALPOWDER') then
            call metalpowders(inpc,textpart,rhcon,nrhcon,elcon,nelcon,
     &        imat,ntmat_,ncmat_,irstrt,istep,istat,n,iperturb,
     &        iline,ipol,inl,ipoinp,inp,ipoinpc,ier,nstate_)
!
         elseif(textpart(1)(1:16).eq.'*POWDERHARDENING') then
            call powderhardenings(inpc,textpart,mpcon,nmpcon,
     &        imat,nmpmat_,irstrt,istep,istat,n,iperturb,
     &        iline,ipol,inl,ipoinp,inp,ipoinpc,ier)
!
...
!
         elseif(textpart(1)(1:22).eq.'*RATE-DEPENDENTPLASTIC') then
            call ratedependentplastics(inpc,textpart,rdpcon,
     &        nmat,ncmat_,iperturb,irstrt,istep,istat,n,
     &        iline,ipol,inl,ipoinp,inp,ipoinpc,ier,nstate_,nelcon)
!
     ...
     &           islavsurf,pslavsurf,clearini,ier,mpcon,nmpcon,nmpmat_)
!
...
!
     ...
     &        icomposite,nelcon,ier)
!
         elseif(textpart(1)(1:15).eq.'*SOLIDIFICATION') then
            call solidifications(inpc,textpart,nmethod,iperturb,isolver,
     &           istep,istat,n,tinc,tper,tmin,tmax,idrct,ithermal,iline,
     &           ipol,inl,ipoinp,inp,alpha,mei,fei,ipoinpc,ctrl,ttime,
     &           ier,enthactrl)
...
!
     ...
     &           ipoinpc,imat,ier,npair,pair2mat,pairmasterslave)
!
...
!
     ...
     &      textpart(1)(1:10).eq.'*PHASEPROP'.or.
     &      textpart(1)(1:16).eq.'*PHASELATENTHEAT'.or.
     &      textpart(1)(1:11).eq.'*PHASECTROL'.or.textpart(1)(1:9)
     &       .eq.'*PHASEZBF'.or.textpart(1)(1:8).eq.'*PHASEGS'.or.
     &      textpart(1)(1:8).eq.'*PHASEYS'.or.
     &       textpart(1)(1:14).eq.'*PHASEHARDNESS')then
...
!
      if(ier.ge.1) then
         ...
         write(*,*) '       input deck: WeICME stops.'
         write(*,*)
         call exit(201)
      endif
!
!     generate ipairmat for enthalpy method ---2021.8.27 by Ye
!
      if(floor(enthactrl(1)+1.d-3).eq.1) then
         do i=1,nmat
            ipairmat(1,i)=-1
            ipairmat(2,i)=-1
            if(nrhcon(i).ne.-1) then
               ipairmat(0,i)=-1
               cycle
            else
               ipairmat(0,i)=i
...
!
            j=1
            do
               if(i.eq.pair2mat(j)) exit
               if(j.gt.npair) then
                  write(*,*) '*ERROR in calinput: interfacial heat'
                  write(*,*) '       transfer properties bugs'
                  call exit(201)
               endif
               j=j+1
            enddo
!
            imat=1
            do
               if(pairmasterslave(1,j).eq.matname(imat))
     &            ipairmat(1,i)=imat
               if(pairmasterslave(2,j).eq.matname(imat))
     &            ipairmat(2,i)=imat
               if((ipairmat(1,i).gt.0).and.(ipairmat(2,i).gt.0))
     &            exit
               if(imat.gt.nmat) then
                  write(*,*) '*ERROR in calinput: interfacial heat'
                  write(*,*) '       transfer properties is not'
                  write(*,*) '       defined correctly'
                  if(ipairmat(1,i).lt.0)
     &               write(*,*) '       not found in material card:',
     &                           pairmasterslave(1,j)
                  if(ipairmat(2,i).lt.0)
     &               write(*,*) '       not found in material card:',
     &                           pairmasterslave(2,j)
                  call exit(201)
               endif
