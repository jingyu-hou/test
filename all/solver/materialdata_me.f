...
!
     ...
     &  xstiff,ncmat_,xstate,nstate_,mpcon,nmpcon,nmpmat_,mpconloc,
     &  rdpcon,gscon,rdploc,gsloc)
!
      implicit none
!
...
!
     ...
     &  iel,iint,mi(*),ncmat_,id,two,seven,nstate_,
...
!
     ...
     &  plconloc(802),dtime,xstate(nstate_),mpcon(2,nmpmat_,*),
     &  mpconloc(800),rdpcon(10,3,*),gscon(30,5,*),rdploc(10,3),
     &  gsloc(30,5)
!
     ...
     &  xstiff,ncmat_,rdpcon,gscon
!
      intent(inout) plconloc,eth,elconloc,elas,mattyp,rho,rdploc,
     &  gsloc 
!
...
!
            ...
            elseif(nelconst.eq.-52) then
               nelconst=5
            elseif(nelconst.eq.-70) then
               nelconst=7
            elseif(nelconst.eq.-71) then
               nelconst=2
            elseif(nelconst.eq.-72) then
               nelconst=2
            elseif(nelconst.eq.-73) then
               nelconst=7
...
!        powder hardening data
         if((kode.eq.-70).or.(kode.eq.-73)) then
           do k=1,nmpcon(2,imat)
             mpconloc(k*2-1)=mpcon(1,k,imat)
             mpconloc(k*2)=mpcon(2,k,imat)
           enddo
         endif
...
!        powder hardening data
         if(kode.eq.-71) then
           do j1=1,10
              do j2=1,3
                 rdploc(j1,j2)=rdpcon(j1,j2,imat)
              enddo
           enddo
           do j1=1,30
              do j2=1,5
                 gsloc(j1,j2)=gscon(j1,j2,imat)
              enddo
           enddo
         endif
!
!        damage data: copy rdploc if damage criteria is set.
!        Only nonlinear transformation models use rdploc; ordinary elastic
!        materials may not have valid rate/damage data in threaded result calls.
         if ((kode.lt.0).and.(kode.gt.-100).and.
     &       (rdpcon(1,2,imat).ne.0.d0)) then
           do j1=1,10
              do j2=1,3
                 rdploc(j1,j2)=rdpcon(j1,j2,imat)
              enddo
...
!     
               itemp=1
!     
               if((kode.lt.-50).and.(kode.gt.-71)) then
                  plconloc(1)=0.d0
                  plconloc(2)=0.d0
                  plconloc(3)=0.d0
