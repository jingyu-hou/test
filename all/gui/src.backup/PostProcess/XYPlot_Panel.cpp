...
XYPlot_Panel::XYPlot_Panel(QWidget *parent) : QDialog(parent), frdVISObj_(0), selectedPointId_(-1)
{
    ...
    QLabel *lb2 = new QLabel(tr("ѡ�����ͣ�"),this);

    PickStyleComb_=new QComboBox(this);
    QStringList strList;
    //strList<<tr("���ӵ�")<<tr("ɾ����")<<tr("ɾ�����е�");
    strList<<tr("���ӵ�")<<tr("ɾ�����е�");
    ...
    m_gbHisCurve->setTitle(tr("��ʷ����"));
    QLabel *lb = new QLabel(tr("����:"), m_gbHisCurve);
    QLabel *lb3 = new QLabel(tr("�������ͣ�"),m_gbHisCurve);
	QLabel *lb22= new QLabel(tr("��������:"), m_gbHisCurve);
    PickCurveComb_=new QComboBox(m_gbHisCurve);
	SetNameComb_=new QComboBox(m_gbHisCurve);
    strList.clear();
    strList<<tr("ʱ����ʷ����")<<tr("λ����ʷ����")<<tr("ѹ������");
    ...
    pickPointBT_->setText(tr("ѡ���"));
    pickPointBT_->setCheckable(true);
    pickPointBT_->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
    createPlotBT_ = new QPushButton(tr("��������"), this);
    
    m_gbPoint=new QGroupBox(this);
    m_gbPoint->setStyleSheet(QString::fromUtf8("::title{color:blue}"));
    m_gbPoint->setTitle(tr("������"));
    
    m_pointSizeLab = new QLabel(tr("���С:"),m_gbPoint);
    m_pointSizeEdit = new QLineEdit(m_gbPoint);
    m_pointClrBtn = new QPushButton(tr("��ɫ"),m_gbPoint);
    m_PointLabChk = new QCheckBox(tr("��ǩ"),m_gbPoint);
    m_PointApplyBtn = new QPushButton(tr("Ӧ��"),m_gbPoint);
    ...
    connect(m_ChgxyzShow,SIGNAL(buttonClicked(int)),this,SLOT(SetChgXYZSlot(int)));
}
...
{
	if(PickCurveComb_->currentText()=="ѹ������")return;
    if (!frdVISObj_)  return;
...
void XYPlot_Panel::Callback_PickPoint(vtkObject *caller, unsigned long, void *clientdata, void*)
{
   ...
   // picker-> (pos,cen,0.1,path,(vtkProp3D*)(path->GetFirstNode()->GetViewProp()), mm);
    ...
    w->UpdataSelectedPoint(picker->GetActor(),str,pointId+1, xyz[0], xyz[1], xyz[2]);//Frd data ��Ӧ��IDֵ��1��ʼ�������Ҫ+1;
...
void XYPlot_Panel::RemoveSelectedPoint(vtkActor *actor,int pointId, double x, double y, double z)
{
    ...
    //if (delPickPointBtn_->isChecked()){//ɾ����
    //    QMapIterator<int,SelectedPointS> it(m_SelectedPointS);
    //    while (it.hasNext()){
    //        it.next();
    //        if(m_SelectedPointS[it.key()].selectedPointId_==TmpSelectP.selectedPointId_){
...
bool XYPlot_Panel::clearAllDisp()
{
    ...
    {
       QMessageBox::StandardButton bt= QMessageBox::warning(this, tr("ɾ��"), tr("ȷ��ɾ����"), QMessageBox::Yes|QMessageBox::No, QMessageBox::Yes);
...
void XYPlot_Panel::CreateXYTimePlot()
{
    ...
    plot_.SetYAxisTitle(var.toAscii().data());
    for (int i=0;i<iPlotNum;i++)
    {
        ...
        plot_.SetLegendLabel(i,(QString("Id=%1").arg(m_SelectedPointS[i].selectedPointId_)).toAscii().data());
...
void XYPlot_Panel::CreateXYPositionPlot()
{
    ...
    plot_.SetGlyphType(0, const_cast<char*>("Point"));
    plot_.SetPlotCurveLines(true);
    //plot_.SetLegendLabel(0,(QString("Id=%1").arg(m_SelectedPointS[i].selectedPointId_)).toAscii().data());
    double dDis=0;
    for (int i=0;i<iPlotNum;i++)
    {
...
void XYPlot_Panel::SaveDataSlot()
{
    ...
    if (PickCurveComb_->currentIndex()==0)//ʱ����ʷ����
    {
        QString data;
	...
	}else if (PickCurveComb_->currentIndex()==1){//λ����ʷ����
        QString data;
        data = "*Position Curve\n";//
        file.write(data.toAscii());
		...
		else if(iDis==3)Direction="����";
		data = "ѹ�����߼������ƣ�"+SetNameComb_->currentText()+"  ѹ������:"+Direction+"\n";//
		file.write(data.toAscii());
		data="ʱ��  ѹ��ֵ\n";/*;*/
		file.write(data.toAscii());
