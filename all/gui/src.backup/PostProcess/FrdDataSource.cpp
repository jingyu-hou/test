...
bool FrdDataSource::Step3_InitResults(_RESULT_STEP_S_ *rStruct)
{
    ...
    for (int i = 0; i < varCount; ++i)
    {
        ...
        strcpy(p, scalar_name.toAscii().data());
        scalarVec_.push_back(p);

        ////save scalar value
        vtkDoubleArray *darry = vtkDoubleArray::New();
        darry->SetName(scalar_name.toAscii().data());
        ...
        else
        {
            ...
            for (int j = 0; j < pointNum; ++j)
            {
                int didx = rStruct->nodeResultBlock.dataIndex.at(j);
                if (pointIdMap_.find(didx) == pointIdMap_.end()) continue;
                int id = pointIdMap_[didx];
...
vtkUnstructuredGrid* FrdDataSource::BuildDisplacementGrid(int gridId, const QString &header)
{
    ...
    int id1 = gSource_->GetScalarIndex(d1.toAscii().constData());
    if (id1 < 0)  return 0;
    int id2 = id1 + 1;
    int id3 = id1 + 2;
...
{
    int id = gSource_->GetScalarIndex(scalar.toAscii().constData());
    return id >= 0;
}
...
bool FrdDataSource::IsScalarValuesDiff(const QString &scalar)
{
    ...
    double *range = gSource_->GetScalarRange(scalar.toAscii().data());
    ...
    if (istyle>2){//--position
        for (map<QString, QStringList>::const_iterator cit = varHeaderScalarMap_.begin(); cit != varHeaderScalarMap_.end(); ++cit)
        {
            ...
            if (HeaderList.contains(scalarName)){  
                ...
                QStringList sparts = scalarName.split("-");
                if (sparts.size() < 3) continue;
                QString scalar(QString("%1:%2").arg(header).arg(sparts.at(2)));
                int scalarId = gSource_->GetScalarIndex(scalar.toAscii().constData());
        ...
        for (map<QString, QStringList>::const_iterator cit = varHeaderScalarMap_.begin(); cit != varHeaderScalarMap_.end(); ++cit)
        {
            ...
            for (int kk=0;kk<varName.size();kk++){
                QStringList hdrParts = header.split("-");
                if (hdrParts.size() >= 2) varTmp<<hdrParts.at(1)+"-"+varName.at(kk);
            }
            
            
            if (varTmp.contains(scalarName)){  
                double time = headerTimeStepMap_[header].toDouble();
                QStringList parts2 = scalarName.split("-");
                if (parts2.size() >= 2) {
                    QString scalar(QString("%1:%2").arg(header).arg(parts2.at(1)));
                int scalarId = gSource_->GetScalarIndex(scalar.toAscii().constData());
                double value = pointResults_[scalarId]->GetValue(pointId);
                resultMap[time] = value;
            }
...
int FrdDataSource::GetPointID(QString strLabel,double x,double y,double z)
{
    ...
    if (strLabel == "undeformed"){//ԭʼ����
        ...
        if (dispGridsMap_[strLabel].find(gridId) == dispGridsMap_[strLabel].end()) return id;
