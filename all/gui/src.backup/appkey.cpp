...
AppKey::AppKey(QObject *parent) : QObject(parent)
{
    ...
    keyUseCount = false;
    keyCount = 10;
	keyUSEHIPAct = true;			//热等静压 Hot Isostatic Pressing(1)
	keyUSEHotExtrusionAct = false; //热挤压 Hot extrusion
	keyUSEForgingAct = true;		//锻造
	keyUSEHeatProcessAct = true;	//热处理 Heat Treatment
	keyUSECastingAct = false;		//铸造
...
void AppKey::loadDefaultEntitlement()
{
    keyData = "";
    ...
    keyUSEHIPAct = true;
    keyUSEHotExtrusionAct = false;
    keyUSEForgingAct = true;
    keyUSEHeatProcessAct = true;
    keyUSECastingAct = false;
}
...
void AppKey::start()
{
	...
	keyUSEHIPAct= (data.at(6) == "1" ? true : false);
	keyUSEHotExtrusionAct= (data.at(7) == "1" ? true : false);
	keyUSEForgingAct= (data.at(8) == "1" ? true : false);
	keyUSEHeatProcessAct= (data.at(9) == "1" ? true : false);
	keyUSECastingAct= (data.at(10) == "1" ? true : false);
...
{
    //采用异或加密,也可以自行更改算法
    QByteArray buffer = data.toAscii();
...
QList<bool> AppKey::PSystem()
{
	start();
	QList<bool> PS;
	PS<<keyUSEHIPAct;
	PS<<keyUSEHotExtrusionAct;
	PS<<keyUSEForgingAct;
	PS<<keyUSEHeatProcessAct;
	PS<<keyUSECastingAct;
    return PS;
}
