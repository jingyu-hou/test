...
{  
  //  Render_Widget *renWidget = Render_Widget::GetInstance();
    ...
    if (type == "PlotOpt")
    {
        ...
        m_PostWidPlotOptDlg->raise();//���ϲ�
        m_PostWidPlotOptDlg->activateWindow();//����
    }
	...
	else if (type == "ReadRest")//
	{
	   ...
	   m_PosWigReadResultDlg->raise();//���ϲ�
	   m_PosWigReadResultDlg->activateWindow();//����
	}
	...
	{
		//m_StackedWidget->setCurrentWidget(m_PosWigFile);�򿪵�ǰFrd�ļ�
	}
	else if (type == "Anim")
	{
		m_StackedWidget->setCurrentWidget(m_PosWigFile);
	...
	else if (type == "RMesh")
	{
		...
		if (!QFileInfo(cmd).exists()) {
			Information_Widget::GetInstance()->ShowInformation("RMesh.exe not found: " + cmd);
		} else {
			QProcess *RMshProcess = new QProcess(this);
				RMshProcess->start(cmd);
				if (!RMshProcess->waitForStarted()) {
				Information_Widget::GetInstance()->ShowInformation("Failed to start RMesh.exe");
				delete RMshProcess;
			}
		}
        #else
		...
		if (!QFileInfo(cmd).exists()) {
			Information_Widget::GetInstance()->ShowInformation("RMesh not found: " + cmd);
		} else {
			QProcess *RMshProcess = new QProcess(this);
			RMshProcess->start(cmd);
			if (!RMshProcess->waitForStarted()) {
				Information_Widget::GetInstance()->ShowInformation("Failed to start RMesh");
				delete RMshProcess;
			}
		}
        #endif
    ...
    for (int kk=0; kk<OnlyHead->size(); kk++) {
        ...
        for (cit; cit != varMap->end(); ++cit){
            ...
            if (curHeader==cmpHead){
                ...
                QStringList hdrParts = cit->first.split("-");
                if (hdrParts.size() >= 2)
                    vars1 << hdrParts.at(1);
...
void QPostPrc::UpDataVisSlot(OptDlgS OptDlg)
{
    ...
    m_optDlgs = OptDlg;
    //m_ActorListData=m_PosWigFile->frdVIS_.GetGridIds();//��ȡ����IdList
...
void QPostPrc::UpDataScalar(ResultVisS ResultVis)
{
    ...
    if (ResultVis.bContour){//��ͼ //��ʾ���κ���ͼ---
        if (strOldName == strOrigCurrentName){//����Ҫ����
            vector<int>::const_iterator it=m_WholeActorData.begin();
             for (it;it!=m_WholeActorData.end();it++){
                 ...
                 m_PosWigFile->frdVIS_.SetContourLevel(*it,ResultVis.m_iClrScalar,"L"+strNumLabel+"-DISP");//��ɫ����
                 m_PosWigFile->frdVIS_.SetContourType(*it,scalar,ResultVis.m_iStyle,"L"+strNumLabel+"-DISP");//��ͼ0 or 2��ֵ��
                 m_PosWigFile->frdVIS_.SetContourVariable(*it,scalar,"L"+strNumLabel+"-DISP");
                 m_PosWigFile->frdVIS_.SetContourOpacity(*it,ResultVis.m_dTrans,"L"+strNumLabel+"-DISP");//͸����

				if (std::find(m_ActorListData.begin(),m_ActorListData.end(),*it)!=m_ActorListData.end()){               
					 if ((*it)==(*m_ActorListData.begin()))
						 m_PosWigFile->frdVIS_.SetContourScalarbarVisible(*it,true,"L"+strNumLabel+"-DISP");//ֻ��ʾһ��Bar
					 else
                 ...
                 if (ResultVis.bContourMinMax){
                     m_PosWigFile->frdVIS_.SetContourRange(*it,ResultVis.contourMin,ResultVis.contourMax,"L"+strNumLabel+"-DISP");//��Χ
                 }
             }
        }else{//����
            vector<int>::const_iterator it=m_WholeActorData.begin();
        ...
        for (it;it!=m_WholeActorData.end();it++){
             m_PosWigFile->frdVIS_.SetContourVisible(*it,scalar,false);
             m_PosWigFile->frdVIS_.SetContourVisible(*it,scalar,false,"L"+strOldName.left(strOldName.indexOf("-")).remove("L",Qt::CaseInsensitive)+"-DISP");//����֮ǰ��
             m_PosWigFile->frdVIS_.SetContourVisible(*it,scalar,false,"L"+strNumLabel+"-DISP");
             m_PosWigFile->frdVIS_.SetContourLevel(*it,ResultVis.m_iClrScalar,"L"+strNumLabel+"-DISP");//��ɫ����
             m_PosWigFile->frdVIS_.SetContourType(*it,scalar,ResultVis.m_iStyle,"L"+strNumLabel+"-DISP");//��ͼ0 or 2��ֵ��
             m_PosWigFile->frdVIS_.SetContourVariable(*it,scalar,"L"+strNumLabel+"-DISP");
    ...
    {
        if (iOrigCurrent==0){//��ʾԭʼ������
            vector<int>::const_iterator it=m_WholeActorData.begin();
            QString header;header.clear(); 
            for (it;it!=m_WholeActorData.end();it++){
                if (std::find(m_ActorListData.begin(),m_ActorListData.end(),*it)!=m_ActorListData.end()){
                    m_PosWigFile->frdVIS_.SetDisplacementVisible(*it,"L"+oldDispName.left(oldDispName.indexOf("-")).remove("L",Qt::CaseInsensitive)+"-DISP",false);//����֮ǰ��
            ...
            if (strOrigCurrentName==oldDispName){
                ...
                if(oldDispName.isEmpty()){
                    ...
                    for (it;it!=m_WholeActorData.end();it++){
                        if (std::find(m_ActorListData.begin(),m_ActorListData.end(),*it)!=m_ActorListData.end()){
                            m_PosWigFile->frdVIS_.SetDisplacementVisible(*it,oldDispName,false); //����֮ǰ��
                            m_PosWigFile->frdVIS_.SetDisplacementVisible(*it,"L"+strOrigCurrentName.left(strOrigCurrentName.indexOf("-")).remove("L",Qt::CaseInsensitive)+"-DISP",true);//��ʾ���ڵ�
                        }else{
                            m_PosWigFile->frdVIS_.SetDisplacementVisible(*it,oldDispName,false); //����֮ǰ��
                            m_PosWigFile->frdVIS_.SetDisplacementVisible(*it,"L"+strOrigCurrentName.left(strOrigCurrentName.indexOf("-")).remove("L",Qt::CaseInsensitive)+"-DISP",false);
                        }                          
