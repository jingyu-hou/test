...
bool QFrdDataPro::ReadFileData(FrdDataVIS &FrdVis,QString fileName,resultFrdS &DataFrdShow)
{
    ...
    QString fileFormat = lists.back();//frd�ļ�
    ...
    QString strId,strTmp;int strFormat = 0;
    ...
    if (!inText.atEnd())
	{
        ...
        {
            if (strList.size() < 4) {
                infoW->ShowInformation("Malformed node data line, skipping");
                line=inText.readLine();
                readNextPrcDataLine(line,strId,strList,strFormat);
                continue;
            }
        ...
        while(strId != "-3")
        {
            ...
            if (strId == "-2")
            {
                if (!tmpElement->DataElem1PointStyle.isEmpty() && (tmpElement->DataElem1PointStyle[0]==4||tmpElement->DataElem1PointStyle[0]==5))//����10���ڵ�Ϊ����
                {
                    line=inText.readLine().simplified();
        ...
        while(line != "9999"&& !inText.atEnd()){//999
            ...
            if (strList.size() >= 4) {
                ...
                if (strId == "100C"){//100C
                if (strList.size() < 7) { line=inText.readLine().simplified(); strId = line.mid(0,4); continue; }
                    strTmp=strList.at(0);
					...
					if (strId =="-4"){//-4
						...
						if(strList.at(1)=="STRESS"){
							Striii=strList.at(1);
							St=strList.at(2).toInt()+5;//����˹Ӧ������һ����������Ӧ���;�ˮѹ����5��
                    ...
                    while(strId != "-3"){
                        ...
                        if (strId == "-1"){//-1
                            ...
                            readNextPrcDataLine(line,strId,strTmpList,1);//��������Ϊ-1����������
                            line=inText.readLine();
...
{ 
    ...
    QSet<QString> seen; QStringList deduped;
    for (int di = 0; di < datS.SetName.size(); ++di) {
        const QString &name = datS.SetName.at(di);
        if (!seen.contains(name)) { seen.insert(name); deduped << name; }
    }
    datS.SetName = deduped;
	m_resultDat=datS;
    date = QDateTime::currentDateTime();
    strTime.append(" ~ ");
    strTime.append(date.toString("hh:mm:ss yy.MM.dd"));
...
void QFrdDataPro::readNextLine2(QString line, QString &strId, QString &strList,QString &Sname)
{
    ...
    if (strList1.size() > 0)
        ...
        strId = "0";
    if (strId == "1" && strList1.size() > 1)
        Sname = strList1.at(1);
    strList.clear();
    int startIdx = (strId == "1") ? 2 : 1;
    for (int i = startIdx; i < strList1.size(); ++i) {
        if (i > startIdx) strList += " ";
        strList += strList1.at(i);
    }
}
