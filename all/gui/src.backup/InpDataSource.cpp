...
bool InpDataSource::InitGridFromFrd(ReadInpResultS *Inp)
{
    ...
    for (int i = 0; i < nPoint; ++i)
	{
        ...
        if (n2D3D==3){//体
		 ...
		 InpPointIdMap_[TmpNodeData.at(0).toInt()]=i+TmpBasePoint.at(0).toInt()-1;
	}
    ...
    QStringList strLcellBaseId=cellPointsList.at(0).split(",");
	CellBaseId_=strLcellBaseId[0].toInt()-1;
    int inpCellType=9;
    for(int i = 0; i < nCell; ++i){
        ...
        if (cell){
            ...
            QStringList tmpStrList = cellPointsList.at(i).split(",");
            for (int j = 0; j < np; ++j){
                idList->SetId(j,InpPointIdMap_[tmpStrList.at(j+1).toInt()]);//tmpStrList.at(j+1).toInt()-1);
            }
            ...
            wholeGrid_->InsertNextCell(cellType_vtk, idList);
            cell->Delete();
        }
    }
