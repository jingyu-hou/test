    ...
    : QDialog(parent)
{
    ...
    QStringList strList;
    strList<<tr("���漯��")<<tr("�����¶�")<<tr("��������")<<tr("����ϵ��")<<tr("�ȴ����ƶ�");
   ...
   /* for (int j=0;j<strList.size();j++){
    ...
    for (int i=0;i<1;i++)
    {
        ...
        TmpCombox->addItem(tr("�½�"));
        TmpCombox->addItem(tr("ɾ��"));
        TmpCombox->setFrame(false);
   ...
   //--delete menu
    ...
    m_popMenu->addAction(tr("ɾ��"),this,SLOT(DelRBtnSlot()));
...
void QHPSystemDlg::WriteHPsysToInp()
{
    ...
    for (int i=0; i<TabRowCnt; i++){
         MyCombox *tmpBtn = (MyCombox*)(m_TableWiget->cellWidget(i, COL_SYS_SYSTEM));//�ƶ�
         QString strCurSysStep=tmpBtn->currentText();//--�ƶ�
         
         QStringList strInclude,strSurfaceNum;
         MyCombox *ff = (MyCombox*)(m_TableWiget->cellWidget(i, COL_SYS_PSET));//��Ԫ
         ...
         if (id !=-1){
             ...
             tmpBtn=(MyCombox*)(m_TableWiget->cellWidget(i, COL_SYS_TEMPER));//����
             id =m_ComboxTemper.key(tmpBtn);
             if (id!=-1){
                 str= m_ComboxTemper[id]->currentText();
                 if (str.contains("constant")){//����Ϊ����
                     ...
                     if(m_TemperateDlg.find(str)!=m_TemperateDlg.end()){
                         ...
                         //emitHPCurve(nodeCurveList);//��������Curve(ȫ��),ͬ���ȵȾ�ѹ����
                         strTmpEnvirTemprat="1";
                         strTmpAmplitudeName=nodeCurveList.strName;
                     }else{;}//new dlgδ����constant��ֵ�趨   
                 }
             }
        
             tmpBtn=(MyCombox*)(m_TableWiget->cellWidget(i, COL_SYS_FILM));//����
             id =m_ComboxFilm.key(tmpBtn);
             if (id!=-1){
                 str= m_ComboxFilm[id]->currentText();
                 if (str=="no"){//��film
                 }else if (str.contains("constant")){//����Ϊ����
             ...
             tmpBtn=(MyCombox*)(m_TableWiget->cellWidget(i, COL_SYS_RADIATE));//����
         ...
         for (int kk=0;kk<nFSize;kk++){
             if (m_TmpNodeHPSystemFList.at(kk).strStep.toLower() != "step1") {//�����¼��ƶ�
         ...
         for (int kk=0;kk<nRSize;kk++){
             if (m_TmpNodeHPSystemRList.at(kk).strStep.toLower() != "step1"&& kk>0) {//�����¼��ƶ�   
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for(int tt=0;tt<ReadInpData.TmpSurfaceInps.strData.size();tt++){
     ...
     for (int mm=0;mm<nHpSysRowSize;mm++){//clear & init
        ...
        int iKKRow=0;
        for (int tt=0;tt<ReadInpData.TmpHPSystemFInps.size();tt++)//FIME���������Ǹ���//--���Ҳ���ʾ
        {
            if (ReadInpData.TmpHPSystemFInps.at(tt).strStyle=="film" && ReadInpData.TmpHPSystemFInps.at(tt).strInclude=="1"){
                //--pset
         ...
         for (int tt=0;tt<ReadInpData.TmpHPSystemRInps.size();tt++){//Radiate���������Ǹ���//--���Ҳ���ʾ
             if (ReadInpData.TmpHPSystemRInps.at(tt).strStyle=="radiate" && ReadInpData.TmpHPSystemRInps.at(tt).strInclude=="1"){
...
void QHPSystemDlg::NextBtnHPSlot()
{
    ...
    if (nRow>0)
    {
        ...
        if (id!=-1){
             ...
             if ((tmpBtn->findText("ɾ��",Qt::MatchCaseSensitive))>0){
                iStepSize--;
            }
            if ((tmpBtn->findText("�½�",Qt::MatchCaseSensitive)>0)){
                iStepSize--;
            }    
        }     
    }
...
{  
    ...
    switch(iCol){
        ...
        case COL_SYS_TEMPER:{
            ...
            if (str.contains("constant")){// == "constant"){//����Ϊ����
                ...
                if (NewDlg->exec()){
                    ...
                    for (int kk=0;kk<tmpBtn->count();kk++){//����
...
void QHPSystemDlg::DelRBtnSlot()
{
    QString str;
     ...
     QMessageBox::StandardButton bt= QMessageBox::question(this, tr("ɾ��"), tr("ȷ��ɾ����"), QMessageBox::Yes|QMessageBox::No, QMessageBox::Yes);
...
//--����HIP������ͬ������HP
//*1.QTemperateDialog,QFilmDialog,QRadiateDialog-- Map ������(str,data);
//*2.combox��ͬ��data;
void QHPSystemDlg::updateHPCurveDataSlot(QStringList combList,InpCurveInpS CurveData)
{
    ...
    for (int kk=0;kk<nRow;kk++){
        ...
        if (id==-1)return;     
        int ind=m_ComboxTemper[id]->currentIndex();
        QString str = m_ComboxTemper[id]->currentText();//��ǰ��ʾ��str��
        m_ComboxTemper[id]->clear();//�������list��
