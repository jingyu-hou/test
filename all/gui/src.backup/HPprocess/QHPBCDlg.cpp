      ...
      : QDialog(parent)
{
    ...
    m_PreBCBtn = new QPushButton(tr("��һ��"));
    m_NextBCBtn = new QPushButton(tr("��һ��"));
    Hlayout->addStretch();
   ...
   /* QTableWidgetItem *tmpItem=new QTableWidgetItem(tr("x��̶�"));
    m_TableWiget->setItem(0,0,tmpItem);

    QTableWidgetItem *tmpItem2=new QTableWidgetItem(tr("y��̶�"));
    m_TableWiget->setItem(1,0,tmpItem2);*/
  //  for (int i=0; i<5;i++)
  //  {
		//QTableWidgetItem *tmpItem=new QTableWidgetItem(tr("����"));
  ...
  //      Tmpcombox->addItem("x��̶�");
  //      Tmpcombox->addItem("y��̶�");
  //      Tmpcombox->addItem("z��̶�");
		//Tmpcombox->addItem("�̶�");
...
void QHPBCDlg::ShowBC(int styleB)
{
    ...
    if (styleB != 2&&Number==0){//�ȵȾ�ѹ ;�ȴ���;
		...
		m_TableWiget->setColumnHidden(0,true);
	}else if(styleB == 2&&Number==0&&NRow>0){//����
       if(NRow>=1)m_TableWiget->item(0,0)->setText(tr("ģ��1"));
	   if(NRow>=2)m_TableWiget->item(1,0)->setText(tr("�����"));
	   if(NRow>=3)m_TableWiget->item(2,0)->setText(tr("�����"));
	   if(NRow>=4)m_TableWiget->item(3,0)->setText(tr("�����"));
	   if(NRow>=5)m_TableWiget->item(4,0)->setText(tr("ģ��2"));
...
void QHPBCDlg::SetInpData(ReadInpResultS ReadInpData)
{
	...
	for(int i=0;i<nBcRowSize1;i++){
		...
		if(str=="��"&&str1!="11,11"&&str2=="")nBcRowSize++;
	}
    if (nRowSize<nBcRowSize){//��ȡ�Ķ������еĴ���Tab������������������
    ...
    if (nRowSize >= nBcRowSize)
    {
       ...
       for (int i=0;i<nBcRowSize;i++){//�õ��б������ж��Ƿ��ظ�����
...
void QHPBCDlg::WriteToBcInps()
{
    ...
    for(int i=0; i<TabRowCnt; i++){
        nodeBCList.strCommentaryName="�߽�����";
...
void QHPBCDlg::AddBtnSlot()//������
{
    int nRow =m_TableWiget->rowCount();
    m_TableWiget->insertRow(nRow);

    MyCombox *NewcomboxPset = new MyCombox(this,nRow);  //�������еĿؼ�
