		...
		for(int i=0;i<nSection;i++){
			...
			for (int ic=0; ic<nSize; ic++){
				...
				if (m_NElData.ElSetInpData.strElSetStyle.at(ic)!=""){
					...
					for (int kk=0;kk<strList.size();kk+=16){//控制输出的数据不超过16个
...
//*************************************************************************************************
//                     剩余节点、节点集合、单元集合、表面集合的输出
//*************************************************************************************************
	NodeInfor2.removeDuplicates();NodeSetInf2.removeDuplicates();ElenentSetInf2.removeDuplicates();//删除节点、节点集合、单元集合的重复信息。
	data = "*Node\n";//节点
...
bool CRWManage::WriteMaterialInpFile(QFile *file, QList<MaterialInpS> m_nodeList, QList<SectionInpS> m_nodeList1,QList<HPPhyInpS> m_nodeHPPhyList)
{
    ...
    for (int i=0; i<nCnt; i++)
    {
        ...
        for (int jj=0; jj<DateVectorNum; jj++)
        {
			...
			if(strName=="plastic"&&PartType=="弹性"||strName=="plastic"&&PartType=="刚体")
				continue;
			...
			if (strName=="metal powder"|| strName=="metalpowder"){//金属粉末
...
bool CRWManage::WriteContactProperties(QFile *file,QList<OCIInpS> m_OPCS,QList<InpCurveInpS> m_nodeList)							  
{
    ...
    file->write(data.toAscii());
	for (int i=0; i<nCnt; i++){
		QString j33=QString::number(i,10);//将整数转为QString
		if(!m_OPCS.at(i).Binding){ 
		     ...
		     if(m_OPCS.at(i).HeatExchangeType=="常数"&&m_OPCS.at(i).HeatExchangeParameter!=""){
			    data="*Gap Conductance\n"+m_OPCS.at(i).HeatExchangeParameter+"\n";//+",0\n"+"0.,1e-5"+"\n";
			    file->write(data.toAscii());
			 }else if(m_OPCS.at(i).HeatExchangeType=="随压力变化"&&m_OPCS.at(i).AmplitudeName!=""){
			    data="*Gap Conductance\n";
			    file->write(data.toAscii());
			 ...
			 }else if(m_OPCS.at(i).HeatExchangeType=="随温度变化"&&m_OPCS.at(i).AmplitudeName!=""){
			    data="*Gap Conductance\n";
			    file->write(data.toAscii());
				for (int ikl=0;ikl<m_nodeList.size();ikl++)
				{
		     ...
		     if(m_OPCS.at(i).FrictionGeneratesHeat!=""){
				...
				QString data1=QString("%1").arg(j);//将整数转为QString
	...
	for (int i=0; i<nCnt; i++){
		...
		if(!m_OPCS.at(i).Binding){
			...
			QString CT;
			if(m_OPCS.at(i).m_ContactType=="面面接触")CT="type=SURFACE TO SURFACE";
...
bool CRWManage::WriteHIPCurveInpFile(QFile *file, QList<InpCurveInpS> m_nodeList)
{
    ...
    for (int i=0;i<m_nodeList.size();i++)
    {
        if (m_nodeList.at(i).ThirdStrTime == 1)//包含时间写入
        {
            data = "** "+m_nodeList.at(i).strCommentaryName + "\n"+
        ...
        }else if (m_nodeList.at(i).ThirdStrTime == 0){//不包含时间写入
            data = "** "+m_nodeList.at(i).strCommentaryName + "\n"+
                "*"+m_nodeList.at(i).strCurveStyle+", name="+m_nodeList.at(i).strName+"\n";
            file->write(data.toAscii());
            file->write(data.toAscii());
...
bool CRWManage::WriteHPTTTCCT(QFile *file, HPTTTInps m_nodeData)
{
    ...
    for (int i=0,j=0; i<nCnt; i++) {
        ...
        if (data.split(",").at(2).toInt()!=0){//数据个数不为0
    ...
    //--写入相平衡
     ...
     for (int i=0,j=0; i<nCnt; i++){
         ...
         if (data.split(",").at(1).toInt()!=0){//数据个数不为0
     ...
     int strCntrAnalyStyle=0,
         ...
         strCntrProp=0,
         strCntrPeriod=0,
         strCntrTTT=0;//StrTTT,默认设置为1

     if (m_nodeData.PhaseCtrolInps.strHeadParam!="" && nCnt>0) //相平衡有数据    
     {
...
/*data = "** Step - "+QString("%1").arg(iStep+1) + m_Solvelist.at(iStep).strCommentaryName + "\n"+
		...
		for(int j1=0;j1<m_OPFS.size();j1++){
			int JJ=m_OPFS.at(j1).ForgingSystemName.toInt();//将字符串转换为整数
			if(JJ==iStep&&m_OPFS.at(j1).NodeSetName.at(j1)!="新建"&&m_OPFS.at(j1).MotionParameterType!="无"&&
				m_OPFS.at(j1).Direction!="No"&&m_OPFS.at(j1).SportType!="No"){
                //data="*Boundary,op=new";
				data="*Boundary";
				if(m_OPFS.at(j1).MotionParameterType=="随时间变化"){
					data+=",amplitude="+m_OPFS.at(j1).CurveName;
				}
				if(m_OPFS.at(j1).SportType=="速度"){
					data+=",type=velocity";
				}
				QString stD;
        ...
        for (int i=0;i<m_nodeHIPsysList.size();i++)
        {
            ...
            if (strTmp == "Boundary"){
                ...
                if (m_nodeHIPsysList.at(i).strAmplitudeName.toLower()!="新建"){//曲线没有新建，则认为没有添加
                    file->write(data.toAscii());
                }                    
            }else if(strTmp == "Dsload"){
                data = "** "+m_nodeHIPsysList.at(i).strCommentaryName + "\n"+
        ...
        //3.变量输出
		nCnt = m_VarList.size();
		if (nCnt>0 ){//多个step下，每个step输出相同 变量
	...
	while(!inText.atEnd()) {
       ...
       while (ret!=-1){//*Element
		   ...
		   if (strList.at(0)=="**partstart"){
               ...
               if (!TmpNodeInpS.strData.empty()&&TmpElInpS.strELType==""){//element中没有数据，说明为第一次不进行保存
                   NodeELSetInps tmpD;
		   ...
		   }else if (strList.at(0)== "*nset"){//*Nset节点集合
              int nNsetSize = strList.size();
		   ...
		   }else if (strList.at(0)== "*elset"){//Elset单元集合
              int nElsetSize = strList.size();
			  QString Str;
		   ...
		   }else if (strList.at(0)== "*surface"){//Surface表面集合
              ...
              while (0==ret){
           ...
           }else if (strList.at(0)=="*amplitude"){//曲线
			   ...
			   for(int i=1;i<Namplit;i++){
				   strTmp = strList.at(i);   
				   if(strTmp.contains("name="))TmpCurve.strName=strTmp.remove("name=", Qt::CaseInsensitive).simplified();//曲线
			   }        
               ret=ReadNextLine(inText, line);
               while (0==ret){
...
//************************************************************************************************************************
		   }else if (strList.at(0)=="*boundary"){//边界条件
                  BCInpS TmpBC;
			   ...
			   for(int i=1;i<strList.size();i++){
				   ...
				   if(str.contains("type=velocity")){
					   ...
					   TmpBC.ParametreType="随时间变化";
				   }
			   }
                   ret=ReadNextLine(inText, line);
			   QStringList BoundaryList0=line.split(",");
			   int Nsss=BoundaryList0.size();
			   if(Nsss<=3||BoundaryList0.at(Nsss-1)=="0")TmpBC.SportType="无";
			   while(ret==0){
                           //TmpSurfaceInps.strData<<line;
			   ...
			   ret=ReadNextLine(inText, line);
                       while (0==ret){
                           ...
                           TmpSys.strCofficient=SysList.at(SysList.size()-1);//最后一个
                           ret=ReadNextLine(inText, line);
                           TmpHIPSystemInps.append(TmpSys);
					   }
		   }else if (strList.at(0)=="*initialconditions"){//初始化
               InitInpS TmpInit;
			   int Ninit;
			   Ninit=strList.size();
               ...
               //TmpInitInps.append(TmpInit); 
		   }else if (strList.at(0)=="*step"){//求解器设置
               ...
               //strTmp=strList.at(1);
               //TmpSlove.strStepInc=strTmp.remove("inc=", Qt::CaseInsensitive).simplified();//最大增益步数
               //strTmp=strList.at(2);
               //TmpSlove.strNlgeom=strTmp.remove("nlgeom=", Qt::CaseInsensitive).simplified();
               ret=ReadNextLine(inText, line);
			   QString SolverType="";
		   ...
		   }else if (strList.at(0)=="*timepoints"){//结果输出间隔
			   //TmpVariableInps
               ret=ReadNextLine(inText, line);
		   ...
		   }else if (strList.at(0)=="*nodefile"||strList.at(0)=="*nodeprint"){//文件结果输出
               ...
               while(1==ret){
				   ...
				   }else if(strKeyStyle=="phaselatentheat"){//潜热
					  ...
					  TmpHPphaseElateHeatInps.append(TmpHPphaseElateHeatList);
				   ...
				   }else if(strKeyStyle=="phaseys"){//屈服强度
				   ...
				   }else if(strKeyStyle=="phasegs"){//晶粒尺寸
				   ...
				   }else if(strKeyStyle=="phasezbf"){//冷速
					   ...
					   TmpHPPhaseZBFInps.append(HPPhaseZBFList);
						 ...
						 strKeyStyle=="dynamicrecrystallization"){
						   ...
						   MaterialList.strSubMCol<< tmpStrM; //--得到当前列数，如果最后一个是空的”,“则去除掉
						   QString StrLP="";
						   while(0==ret){
							   QStringList tmpL1;
           ...
           //}else if (strList.at(0)=="*Physical Constants"){
               ...
               if(HPSystemFList.strElsetName==""){//未进入“while（ret =0），film下无参数”
                    TmpHPSystemFInps.append(HPSystemFList);
               }
               ...
               if(HPSystemRList.strElsetName==""){//未进入“while（ret =0），radiation下无参数”
                    TmpHPSystemRInps.append(HPSystemRList);
               }
				   ...
				   if(strKeyStyle=="friction"){
				    ...
				    ret = ReadNextLine(inText,line);
				       ...
				       ret = ReadNextLine(inText,line);
						...
						readline=line.split(",");
						if(readline.size()==1){
							RECPPInInp.HeatExchangeType="常数";
							RECPPInInp.HEParam=readline.at(0).simplified();
							ret = ReadNextLine(inText,line);
						}else if(readline.size()==3&&readline.at(1).simplified()==""){
							RECPPInInp.HeatExchangeType="随温度变化";
							RECPPInInp.strCurveName="heatexchange"+QString::number(TmpCPPInInp.size(),10);//将整数转为QString
							while(ret==0){
								readline=line.split(",");
							...
							RECPPInInp.HeatExchangeType="随压力变化";
							RECPPInInp.strCurveName="heatexchange"+QString::number(TmpCPPInInp.size(),10);//将整数转为QString
							while(ret==0){
								readline=line.split(",");
								RECPPInInp.strDataY<<readline.at(0);
...
//------------------------------------------------------------
//--Read Inp From .INP file  02(用于合并inp文件的读取)
//------------------------------------------------------------
...
int CRWManage::ReadNextLine(QTextStream &TextStreamData,QString &DataLine)
{
	...
	DataLine.remove(QRegExp("\\s"));//删除空格
	if(DataLine.right(1)==",")DataLine.chop(1);//去除尾部的逗号
	DataLine=DataLine.toLower();//改为小写
	...
	while (iContains>=0){//在本行5个之前中包含2个“**“
		DataLine=TextStreamData.readLine().simplified();
...
/*QTime t120;
	...
	for(int i=0;i<curData.gmshNSetData.strData.size();i++){
		...
		for(int j=0;j<BCD.size();j++){
			...
			for(int k=0;k<fs.size();k++){
				...
				int afwsw=NodeNu.value(st02);//使*NSET中节点编号与*NODE中节点号相同
	...
	if(styleType==1||styleType==2){
		for (int kk=0;kk<nSize;kk++){
			...
			//以下几行实现*ELEMENT单元信息中节点编号与*NODE中节点编号同步
			...
			for(int ijk=0;ijk<NumberE1;ijk++){
				...
				for(int ijk2=1;ijk2<stl23.size();ijk2++){
					...
					int afwsw=NodeNu.value(stll);//使*ELEMENT中节点编号与*NODE中节点编号同步
				...
				if(tmpType==1&&styleType==1||tmpType==2&&styleType==2){
					...
					ElementCmp.insert(strFirst,NStr);//保存表面单元信息
				}
				//tmpELdata32.strData.append (NStr);
			}
			//以上几行实现*ELEMENT单元信息中节点编号与*NODE中节点编号同步
	...
	for(int kk=0;kk<Nsize;kk++)
	{
		...
		ElementNumber=kk+1;
		ElementEu.insert(Str.at(0),ElementNumber);//重新编号前后的映射关系
		QString j33=QString::number(ElementNumber,10);//将数据转换成QString
	...
	if (styleType == 2||styleType == 1){//2d/3d
		...
		for (int kk=0; kk<gElSetInpSData.strData.size(); kk++){//单元集合中集合数量的循环 
			QString STL2_0;
			...
			for (int mm=0; mm<strListGElSet.size(); mm++){ //单元集合中单元数量的循环 
                QString strId = strListGElSet.at(mm).simplified(); 
                int iSn=0;
                QCoreApplication::processEvents();
				if(ElementCmp.find(strId)!= ElementCmp.end()){//存在ElementCmp.contains(strId)
	...
	//以上几行用于删除表面集合建立生成的节点集合

	//以下几行用于删除表面集合建立生成的单元集合
...
bool CRWManage::ELinclude(QString strType, QString strSrc, QString strDis, int &iSurf)
{
    ...
    for (int kk=0; kk<nDis; kk++){
        ...
        if (id==-1)return false;//没有找到不存在于该element行中;
        else if (id==0)continue;//存在,但为第一个序号，则继续寻找;
        indX.append(id);
    }
    qSort(indX.begin(),indX.end());
    int iBaseNum=0;
    if (strType=="c3d6"){iBaseNum = 4;//3个或者4个
    }else if(strType=="c3d15"){iBaseNum = 8;//6个或8个
    }else if(strType=="c3d8"||strType=="c3d8r"){//4个点
        iBaseNum = 4;
        iSurf=CalcSn(indX,iBaseNum,strType);
    }else if(strType=="c3d20"||strType=="c3d20r"){iBaseNum = 8;
    }else if(strType=="cps3"||strType=="cpe3"||strType=="cax3"){//三角形（三个点，每个面2个点）
        iBaseNum = 2;
        iSurf=CalcSn(indX,iBaseNum,strType);
    }else if(strType=="cps6"||strType=="cpe6"||strType=="cax6"){iBaseNum = 3;
    }else if(strType=="cpe4"||strType=="cps4r"||strType=="cpe4"||strType=="cpe4r"||strType=="cax4"||strType=="cax4r"){//4变形
        iBaseNum = 2;
        iSurf=CalcSn(indX,iBaseNum,strType);
...
QString CRWManage::sort(int number,QString Str)
{
	...
	int temp;
	for(int i=0;i<number;i++){
		numberL[i]=Str.split(",").at(i).toInt();
	}
