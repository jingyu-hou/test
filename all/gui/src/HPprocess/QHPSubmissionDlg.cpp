...
QHPSubmissionDlg::QHPSubmissionDlg(QWidget *parent)
    : QDialog(parent)
{
    CreatGUI();
}

QHPSubmissionDlg::~QHPSubmissionDlg()
{
...
void QHPSubmissionDlg::CreatGUI()
{
    ...
    m_KillBtn = new QPushButton(tr("终止计算"));
	...
	mainLayout->addWidget(m_KillBtn,4,0,1,1);
	...
	connect(m_KillBtn,SIGNAL(clicked()),this,SLOT(KillolverSlot()));
	
}
void QHPSubmissionDlg::SetPathSlot()
{
}
//--向主窗口发送 保存求解命令
void QHPSubmissionDlg::SaveHPInpSlot()
{
    emit emitHpInpSave();
}
//--向主窗口发送运行求解器命令 
void QHPSubmissionDlg::RunSolverSlot()
{
    emit emitHpInpRun();
}
void QHPSubmissionDlg::CheckSlot()
{
    emit emitCheck();
}
void QHPSubmissionDlg::KillolverSlot()
{
	emit emitHipInpKill();
}
