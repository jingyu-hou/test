        ...
        : QDialog(parent)
{
    ...
    m_PreSystemBtn = new QPushButton(tr("上一步"));
	m_NextSystemBtn = new QPushButton(tr("下一步"));
    Hlayout1->addStretch();
    ...
    int iniRowCnt=1;
    QStringList strList;
	strList<<"部件"<<"节点集合"<<"运动类型"<<"运动方向"<<"运动参数类型"<<"运动参数"<<"热边界名称";
    QVBoxLayout *VLayout = new QVBoxLayout();
    ...
    QTableWidgetItem *tmpItem2=new QTableWidgetItem(tr("部件"));
    m_TableWiget->setItem(0,0,tmpItem2);

	MyCombox *tmpCombox01=new MyCombox(this,0);
    tmpCombox01->addItem(tr("新建"));
	...
	tmpCombox1->addItem(tr("无运动"));
    tmpCombox1->addItem(tr("位移"));
	tmpCombox1->addItem(tr("速度"));
    ...
    tmpCombox3->addItem(tr("常数"));
	tmpCombox3->addItem(tr("随时间变化"));
...
void QForgingSystemDlg::SetInpData(ReadInpResultS ReadInpData)
{
	...
	for(int i=0;i<nBcRowSize1;i++){
		...
		//if(str=="6"&&str2!=""&&Step>nFSystem)nFSystem=Step;//多工步需要取消改行注释
		if(str!=""&&str2!=""&&Step>nFSystem)nFSystem=Step;//多工步需要注释该行
		}
	...
	for(II=0;II<nRowSize;II++){
		...
		m_NodeSet.at(II)->addItem("新建");
		m_NodeSet.at(II)->addItems(Nodeset);
		//m_NodeSet.at(II)->setItemData(II,"0",(Qt::UserRole+3));//0代表点集合,1代表单元集合,2代表面集合
		connect(m_NodeSet.at(II),SIGNAL(clicked(int,int)),this,SLOT(ComboxSlotHighlight(int,int)));
	}

	//查找集合、运动类型、运动方向、运动参数类型、运动参数类型并显示
...
{  
  ...
  //--设置属性
  iTmp= m_NodeSet.at(comWigId)->itemData(index0,Qt::UserRole+3).toInt();//当前所选combox
...
void QForgingSystemDlg::WriteToFSInps()
{
	...
	for(int i=0; i<TabRowCnt; i++){
		...
		if(OputFSInf.MotionParameterType=="常数"){
 ...
 void QForgingSystemDlg::ShowFS(QStringList m_ThermalBoundaryName)
{
	...
	for(int i=0;i<NRow;i++){
		...
		if(id==-1)id=ff1->findText("无");
		ff1->setCurrentIndex(id);//根据给定索引号使该combox显示索引号下的值
	}
