! The multi-order-parameter phase field modeling for grain growth.
! the sparse matrix data is used for saving the computational time.
! writen by Namin Xiao. nmxiao@outlook.com

MODULE Constant
  IMPLICIT none

  INTEGER, PARAMETER :: SBL = SELECTED_REAL_KIND (p=6, r=37)             !Single data kind
  INTEGER, PARAMETER :: DBL = SELECTED_REAL_KIND (p=13)                  !Double data kind
!----------------------------------------------------------------------+
  REAL (KIND = DBL), PARAMETER :: PI = 3.1415926D0
  REAL (KIND = DBL), PARAMETER :: R = 8.314D0
  REAL (KIND = DBL), PARAMETER :: KB = 1.380658D-23
  REAL (KIND = DBL), PARAMETER :: LOGE = 2.71828183D0

  REAL (KIND = DBL), PARAMETER :: DX = 1.0D-6
  REAL (KIND = DBL), PARAMETER :: DY = DX
  REAL (KIND = DBL), PARAMETER :: DT = 5.0D-3
  REAL (KIND = DBL), PARAMETER :: KELVIN_T = 880.0D0+273.15D0
  INTEGER, PARAMETER :: TSTEP = 500
  INTEGER, PARAMETER :: TINTERVAL = 100
  INTEGER, PARAMETER :: BC = 1

  INTEGER, PARAMETER :: ANISOTROPY_ORI = 0 ! if the misorientation anisotropy is considered, replace 0 by 1

  REAL (KIND = DBL), PARAMETER :: BV = 2.56D-10
  REAL (KIND = DBL), PARAMETER :: DELTADB = 1.1D-13
  REAL (KIND = DBL), PARAMETER :: QB = 174000.0D0
  REAL (KIND = DBL), PARAMETER :: SGB_0 = 0.835D0
  REAL(KIND = DBL), PARAMETER :: UGB_0 = BV*DELTADB/KB/KELVIN_T*exp(-QB/R/KELVIN_T)
  !REAL(KIND = DBL), PARAMETER :: UGB_0 = 4.1E5*exp(-420000.0/R/KELVIN_T)
  !REAL (KIND = DBL), PARAMETER :: UGB_0 = 8.9D-5*BV*BV/KB/KELVIN_T*exp(-208000.0D0/R/KELVIN_T)*1.5D0
  !u_gb = 1.0E-10
  REAL (KIND = DBL), PARAMETER :: GAMMA_0 = 1.5D0
  REAL (KIND = DBL), PARAMETER :: LGB = 4.D0*DX
  REAL (KIND = DBL), PARAMETER :: THETA_0 = 20.0D0*PI/180.0D0
  REAL (KIND = DBL), PARAMETER :: TOLERANCE = 1.0D-3
  INTEGER, PARAMETER :: NNEI = 1
END MODULE Constant

MODULE SparseDataStructure
  USE Constant
  IMPLICIT none
  SAVE

  TYPE node
    REAL (KIND = DBL) :: eta !value of the orde parameter
    INTEGER :: eta_id !the id of the order parameter
    integer :: region_id
    TYPE (node), POINTER :: next
  END TYPE node

  TYPE sparse_struct
    TYPE (node), POINTER :: head_ptr !points to the head of the list
    TYPE (node), POINTER :: copy_ptr !used to read through the list
    INTEGER :: list_size !number of order parameters at list spatial position
  END TYPE sparse_struct

  !REAL (kind=DBL), DIMENSION (:,:), ALLOCATABLE :: neighbor_array !contains the neighboring grid point values
  REAL (kind=DBL), DIMENSION (:,:,:), ALLOCATABLE :: sum_eta !contains the neighboring grid point values
  REAL (kind=DBL), DIMENSION (:,:,:), ALLOCATABLE :: kp, lp, yp, mp !contains the neighboring grid point values
  INTEGER, ALLOCATABLE, DIMENSION (:,:,:) :: sharp_ori
  INTEGER, ALLOCATABLE, DIMENSION (:,:,:) :: gb_stat

  TYPE (node), POINTER :: current_ptr !used to allocate new order parameters to the list
  TYPE (node), POINTER :: previous_ptr !used to allocate new order parameters to the list
  TYPE (node), POINTER :: secondary_ptr !used when current_ptr is in use holding values for a main loop
  TYPE (sparse_struct), ALLOCATABLE, TARGET :: grid(:,:,:) !contains all the order parameters in x,y,z space

  LOGICAL :: ierr
  INTEGER :: ioerr
  INTEGER :: x, y, z, t, matrix_q_number, total_q_number
  CHARACTER (LEN = 32) :: output_path, directory_name
END MODULE SparseDataStructure

MODULE ExternalSub
  CONTAINS
  SUBROUTINE IncreaseArrayRank_1D(a, dynamic_array)
    IMPLICIT none
    INTEGER, INTENT (in) :: a
    INTEGER :: i
    INTEGER, ALLOCATABLE, DIMENSION (:), INTENT (inout) :: dynamic_array
    INTEGER, ALLOCATABLE, DIMENSION (:) :: temp

    ALLOCATE (temp(a-1))

    temp = dynamic_array

    DEALLOCATE (dynamic_array)

    ALLOCATE (dynamic_array(a))

    FORALL (i=1:a-1)
        dynamic_array(i) = temp(i)
    END FORALL

    DEALLOCATE (temp)

  END SUBROUTINE

  SUBROUTINE IncreaseArrayRank_3D(a, b, c, dynamic_array)
    IMPLICIT none
    INTEGER, INTENT (in) :: a, b, c
    INTEGER :: k,i,j
    REAL, ALLOCATABLE, DIMENSION (:,:,:), INTENT (inout) :: dynamic_array
    REAL, ALLOCATABLE, DIMENSION (:,:,:) :: temp

    ALLOCATE (temp(c-1,a,b))

    temp = dynamic_array

    DEALLOCATE (dynamic_array)

    ALLOCATE (dynamic_array(c,a,b))

    DO j=1,b
        DO i=1,a
            DO k=1,c-1
                dynamic_array(k,i,j) = temp(k,i,j)
            END DO
        END DO
    END DO

    DEALLOCATE (temp)

  END SUBROUTINE

END MODULE ExternalSub

PROGRAM main
  USE SparseDataStructure
  USE Constant
  USE IFPORT
  IMPLICIT none

  REAL (KIND = DBL) :: start_time,finish_time

  directory_name = 'Output'
  output_path = '.\\'//trim(adjustl(directory_name))//'\\'
  ierr = MAKEDIRQQ (trim(directory_name))

  x = 64
  y = 64
  z = 64

  ALLOCATE (grid(x,y,z))

  ALLOCATE (sum_eta(x,y,z))
  ALLOCATE (kp(x,y,z))
  ALLOCATE (lp(x,y,z))
  ALLOCATE (yp(x,y,z))
  ALLOCATE (mp(x,y,z))
  ALLOCATE (sharp_ori(x,y,z))
  ALLOCATE (gb_stat(x,y,z))

  gb_stat = 0

  CALL ReadFile()
  CALL AnisotropyInterface()
  WRITE (*,*) 'Initialization is finished.'

  start_time = DCLOCK()

  DO t = 1, TSTEP
    CALL PhaseEvolution()
    IF (mod(t, TINTERVAL) == 0) CALL Output()
  END DO

  finish_time = DCLOCK()
  WRITE (*,*) 'The total time =', finish_time-start_time, ' seconds'

  DEALLOCATE (grid)
  DEALLOCATE (sum_eta)

  DEALLOCATE (kp)
  DEALLOCATE (lp)
  DEALLOCATE (yp)
  DEALLOCATE (mp)
  DEALLOCATE (sharp_ori)
  DEALLOCATE (gb_stat)

END PROGRAM main

SUBROUTINE ReadFile()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  INTEGER :: i, j, k, l, m, count_i, first_on_list, inside_loop
  INTEGER :: ii, jj, kk, nei, nx, ny, nz, id_exists
  INTEGER, ALLOCATABLE, DIMENSION (:,:,:) :: input_ori
  INTEGER, ALLOCATABLE, DIMENSION (:) :: ori_list

  ALLOCATE (input_ori(x,y,z))

  open(unit=10, file="64_ORI.raw", form='binary', status='old', action='read', iostat=ioerr)
  IF (ioerr /= 0) THEN
    WRITE (*,*) "cannot find the input file!"
  ELSE
    DO k = 1, z
      DO j = 1, y
        DO i = 1, x
          READ (10) input_ori(i,j,k)
        END DO
      END do
    end do
  END IF
  CLOSE (10)

  matrix_q_number = 1
  ALLOCATE (ori_list(matrix_q_number))
  ori_list(1) = input_ori(1,1,1)

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        count_i = 0
        DO l = 1, matrix_q_number
          IF (input_ori(i,j,k) == ori_list(l)) THEN
            count_i = 1
          END IF
        END DO
        IF (count_i == 0) THEN
          matrix_q_number = matrix_q_number + 1
          CALL IncreaseArrayRank_1D(matrix_q_number, ori_list)
          ori_list(matrix_q_number) = input_ori(i,j,k)
        END if
      end do
    END DO
  END do

  allocate(region_index(matrix_q_number))
  region_index = 0

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        NULLIFY (grid(i,j,k)%head_ptr) !initially nullify list (empty)
      end do
    END DO
  END do

  total_q_number = matrix_q_number

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        grid(i,j,k)%list_size = 0
        DO l = 1, matrix_q_number
          IF (ori_list(l) == input_ori(i,j,k)) THEN
            IF (.NOT. ASSOCIATED(grid(i,j,k)%head_ptr)) THEN
              ALLOCATE (grid(i,j,k)%head_ptr)
              current_ptr => grid(i,j,k)%head_ptr

              current_ptr%eta = 1.0
              current_ptr%eta_id = l
              grid(i,j,k)%list_size = grid(i,j,k)%list_size + 1
              NULLIFY(current_ptr%next)
            else
              ALLOCATE (current_ptr%next)
              current_ptr => current_ptr%next

              current_ptr%eta = 1.0
              current_ptr%eta_id = l
              grid(i,j,k)%list_size = grid(i,j,k)%list_size + 1
              NULLIFY(current_ptr%next)
            end if
          end if
        end do
      end do
    end do
  end do

  CALL UpdateSparseData()

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        do
          IF (.NOT.ASSOCIATED(grid(i,j,k)%head_ptr)) exit
          if (grid(i,j,k)%head_ptr%region_id == 0) then
            indice = 0
            DO kk = -1, 1
              DO jj = -1, 1
                DO ii = -1, 1
                  id_exists = 0
                  nx = i+ii
                  ny = j+jj
                  nz = k+kk
                  IF (nx == 0) nx = 1
                  IF (nx == x+1) nx = x
                  IF (ny == 0) ny = 1
                  IF (ny == y+1) ny = y
                  IF (nz == 0) nz = 1
                  IF (nz == z+1) nz = z
                  current_ptr => grid(nx,ny,nz)%head_ptr
                  do
                    IF (.NOT.ASSOCIATED(current_ptr)) exit
                    if ((current_ptr%eta_id == grid(i,j,k)%head_ptr%eta_id) && (current_ptr%region_id /= 0)) then
                      grid(i,j,k)%head_ptr%region_id = current_ptr%region_id
                      indice = 1
                      exit
                    end if
                    current_ptr => current_ptr%next
                  end do
                end do
              end do
            end do
            if (indice == 0) then
              grid(i,j,k)%head_ptr%region_id = region_index(grid(i,j,k)%head_ptr%eta_id) + 1
              region_index(grid(i,j,k)%head_ptr%eta_id) = region_index(grid(i,j,k)%head_ptr%eta_id) + 1
            end if
          end if
          grid(i,j,k)%head_ptr => grid(i,j,k)%head_ptr%next
        end do
      end do
    end do
  end do

  do l = 1, matrix_q_number
    if(region_index(l) /= 0) then
      do m = 1, region_index(l)
        x_min = 0
        x_max = 0
        y_min = 0
        y_max = 0
        z_min = 0
        z_max = 0
        DO k = 1, z
          DO j = 1, y
            DO i = 1, x
              do
                IF (.NOT.ASSOCIATED(grid(i,j,k)%head_ptr)) exit
                if ((grid(i,j,k)%head_ptr%eta_id == l) && (grid(i,j,k)%head_ptr%region_id == m)) then
                  if (i<x_min) x_min = i
                  if (i>x_max) x_max = i
                  if (j<y_min) y_min = j
                  if (j>y_max) y_max = j
                  if (k<z_min) z_min = k
                  if (k>z_max) z_max = k
                end if
                grid(i,j,k)%head_ptr => grid(i,j,k)%head_ptr%next
              end do
            end do
          end do
        end do

        allocate(eta_fft((x_min-1):(x_max+1), (y_min-1):(y_max+1), (z_min-1):(z_max+1))

        eta_fft = 0.D0
        DO k = x_min, x_max
          DO j = y_min, y_max
            DO i = z_min, z_max
              do
                IF (.NOT.ASSOCIATED(grid(i,j,k)%head_ptr)) exit
                if ((grid(i,j,k)%head_ptr%eta_id == l) && (grid(i,j,k)%head_ptr%region_id == m)) then
                  eta_fft(i,j,k) = grid(i,j,k)%head_ptr%eta
                end if
                grid(i,j,k)%head_ptr => grid(i,j,k)%head_ptr%next
              end do
            end do
          end do
        end do

        call FFT_evolution(eta_fft)

  CALL UpdateSumEta()
  CALL UpdateSharpOri()

  DEALLOCATE (input_ori)
  DEALLOCATE (ori_list)

END SUBROUTINE ReadFile

SUBROUTINE NeighborNode(a, b, c, neighbor_array)
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  INTEGER :: i, j, k, nx, ny, nz, a, b ,c
  TYPE (node), POINTER :: neighbor_ptr !used exclusively in the neighbor subroutine since it is called from other pointer loops

  REAL (kind=DBL), DIMENSION (-1:1,-1:1,-1:1) :: neighbor_array !contains the neighboring grid point values
  !ALLOCATE (neighbor_array(-1:1,-1:1))
  neighbor_array = 0.d0

  DO k = -1, 1
    DO j = -1, 1
      DO i = -1, 1
        nx = a+i
        ny = b+j
        nz = c+k
        IF (nx == 0) nx = x
        IF (nx == x+1) nx = 1
        IF (ny == 0) ny = y
        IF (ny == y+1) ny = 1
        IF (nz == 0) nz = z
        IF (nz == z+1) nz = 1

        neighbor_ptr => grid(nx,ny,nz)%head_ptr
        DO
          IF (.NOT. ASSOCIATED(neighbor_ptr)) EXIT
          IF (neighbor_ptr%eta_id == grid(a,b,c)%copy_ptr%eta_id) THEN
            neighbor_array(i,j,k) = neighbor_ptr%eta
            EXIT
          END IF
          neighbor_ptr => neighbor_ptr%next
        END do
      end do
    END DO
  END do

    !DEALLOCATE (neighbor_array)

END SUBROUTINE NeighborNode

SUBROUTINE UpdateSparseData()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  INTEGER :: i, j, k, ii, jj, kk, nx, ny, nz, first_on_list, inside_loop, id_exists
  REAL (kind=DBL), DIMENSION (-1:1,-1:1,-1:1) :: neighbor_array !contains the neighboring grid point values

  CALL CopyData()

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        first_on_list = 1
        DO
          IF (.NOT.ASSOCIATED(grid(i,j,k)%copy_ptr)) EXIT

          !Insert the order parameter
          IF (grid(i,j,k)%copy_ptr%eta >= TOLERANCE) THEN
          !order parameters from nearest & second nearest neighbors are added here
          !but only added if they don't already exist and aren't in the ghost boundary
            DO kk = -1, 1
              DO jj = -1, 1
                DO ii = -1, 1
                  id_exists = 0
                  nx = i+ii
                  ny = j+jj
                  nz = k+kk
                  IF (nx == 0) nx = x
                  IF (nx == x+1) nx = 1
                  IF (ny == 0) ny = y
                  IF (ny == y+1) ny = 1
                  IF (nz == 0) nz = z
                  IF (nz == z+1) nz = 1
                  current_ptr => grid(nx,ny,nz)%head_ptr
                  DO
                  !check to see if order parameter already exists at neighboring indices
                    IF (.NOT.ASSOCIATED(current_ptr)) EXIT
                    !IF (((i==1).AND.(ii<0)).OR.((i==x).AND.(ii>0))) id_exists = 1 !don't add to ghost boundary condition indices
                    !IF (((j==1).AND.(jj<0)).OR.((j==y).AND.(jj>0))) id_exists = 1 !don't add to ghost boundary condition indices

                    IF (current_ptr%eta_id == grid(i,j,k)%copy_ptr%eta_id) THEN
                      id_exists = 1
                      EXIT
                    END IF
                    current_ptr => current_ptr%next
                  END DO

                  !order parameter doesn't exist so add the order parameter here
                  IF (id_exists == 0) THEN
                    ALLOCATE (current_ptr)
                    current_ptr%eta_id = grid(i,j,k)%copy_ptr%eta_id
                    current_ptr%eta = 0.d0
                    current_ptr%next => grid(nx, ny, nz)%head_ptr !point to previous head memory position
                    grid(nx, ny, nz)%head_ptr => current_ptr !update head of list
                  END IF
                END DO
              END do
            end do
          END IF

          inside_loop = 0

          IF (grid(i,j,k)%copy_ptr%eta < TOLERANCE) THEN
            !check if all nearest neighbors are below tolerance as well, if not then don't remove
            CALL NeighborNode(i,j,k,neighbor_array)
            IF (MAXVAL(neighbor_array) < TOLERANCE) THEN
              IF (first_on_list == 1) THEN
                current_ptr => grid(i,j,k)%copy_ptr
                grid(i,j,k)%head_ptr => grid(i,j,k)%copy_ptr%next
                grid(i,j,k)%copy_ptr => grid(i,j,k)%copy_ptr%next
                DEALLOCATE (current_ptr) !prevent memory leak by releasing memory
                inside_loop = 1
              ELSE
                current_ptr => grid(i,j,k)%copy_ptr
                grid(i,j,k)%copy_ptr => grid(i,j,k)%copy_ptr%next
                previous_ptr%next => grid(i,j,k)%copy_ptr !link last position with next position
                DEALLOCATE (current_ptr) !prevent memory leak by releasing memory
                inside_loop = 1
              END IF
            END IF
          END IF

          IF (inside_loop == 0) THEN
            first_on_list = 0
            previous_ptr => grid(i,j,k)%copy_ptr
            grid(i,j,k)%copy_ptr => grid(i,j,k)%copy_ptr%next
          END IF
        end do
      END DO
    END DO
  END DO

  grid(:,:,:)%list_size = 0
  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        current_ptr => grid(i,j,k)%head_ptr
        DO
          IF (.NOT.ASSOCIATED(current_ptr)) EXIT
          grid(i,j,k)%list_size = grid(i,j,k)%list_size + 1
          current_ptr => current_ptr%next
        END DO
      END DO
    END do
  end do

END SUBROUTINE UpdateSparseData

SUBROUTINE CopyData()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  integer :: i, j, k

  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        grid(i,j,k)%copy_ptr => grid(i,j,k)%head_ptr
      end do
    END DO
  END DO

END SUBROUTINE CopyData

SUBROUTINE UpdateSumEta()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  integer :: i, j, k, l

  sum_eta = 0.0

  CALL CopyData()

  !$OMP PARALLEL DO private(i, j, k, l) shared(grid) REDUCTION(+:sum_eta)
  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        DO l = 1, grid(i,j,k)%list_size
          IF (.NOT.ASSOCIATED(grid(i,j,k)%copy_ptr)) EXIT
          !ASSOCIATE (eta => grid(i,j,k)%copy_ptr%eta)
            sum_eta(i,j,k) = sum_eta(i,j,k) + grid(i,j,k)%copy_ptr%eta*grid(i,j,k)%copy_ptr%eta
            grid(i,j,k)%copy_ptr => grid(i,j,k)%copy_ptr%next
          !END ASSOCIATE
        END do
      end do
    END DO
  END do
  !$OMP end PARALLEL DO
END SUBROUTINE UpdateSumEta


SUBROUTINE PhaseEvolution()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  integer :: i, j, k, l, m, ii, jj, kk
  real :: f0, laplacian

  REAL (kind=DBL), DIMENSION (-1:1,-1:1,-1:1) :: neighbor_array !contains the neighboring grid point values
  !integer :: start_time,finish_time, np_rate

  CALL CopyData()

  !call system_clock(count_rate=np_rate)
  !call system_clock(count=start_time)

  !$OMP PARALLEL DO private(i, j, k, l, f0, laplacian, ii, jj, kk, neighbor_array) shared(grid, kp, lp, mp, sum_eta)
  DO k = 1, z
    DO j = 1, y
      DO i = 1, x
        f0 = 0.D0
        DO l = 1, grid(i,j,k)%list_size
        IF (.NOT.ASSOCIATED(grid(i,j,k)%copy_ptr)) EXIT
          !ASSOCIATE (eta => grid(i,j)%copy_ptr%eta)
            f0 = mp(i,j,k)*(-grid(i,j,k)%copy_ptr%eta + grid(i,j,k)%copy_ptr%eta*grid(i,j,k)%copy_ptr%eta*grid(i,j,k)%copy_ptr%eta &
            & + 2.D0*yp(i,j,k)*grid(i,j,k)%copy_ptr%eta*(sum_eta(i,j,k) - grid(i,j,k)%copy_ptr%eta*grid(i,j,k)%copy_ptr%eta))

            CALL NeighborNode(i,j,k,neighbor_array)
            laplacian = 0.D0
            !DO kk = -1,1
            !  DO jj = -1,1
            !    DO ii = -1,1
            !      IF ((ii == 0) .or. (jj == 0) .or. (kk == 0)) THEN
            !        laplacian = laplacian - 0.5D0*(neighbor_array(ii,jj,kk)-neighbor_array(0,0,0))
            !      ELSE
            !        laplacian = laplacian - 0.25D0*(neighbor_array(ii,jj,kk)-neighbor_array(0,0,0))
            !      END if
            !    end do
            !  END DO
            !END DO

            !laplacian = 8.D0*(neighbor_array(-1,-1,0)+neighbor_array(-1,0,-1)+neighbor_array(-1,0,1)+neighbor_array(-1,1,0) &
            !& +neighbor_array(0,-1,-1)+neighbor_array(0,-1,1)+neighbor_array(0,1,-1)+neighbor_array(0,1,1) &
            !& +neighbor_array(1,-1,0)+neighbor_array(1,0,-1)+neighbor_array(1,0,1)+neighbor_array(1,1,0)) &
            !& - 11.D0 * (neighbor_array(-1,0,0)+neighbor_array(0,-1,0)+neighbor_array(0,0,-1)+neighbor_array(0,0,1) &
            !& +neighbor_array(0,1,0)+neighbor_array(1,0,0)) + 30.D0*neighbor_array(0,0,0)

            laplacian = neighbor_array(-1,0,0)+neighbor_array(0,-1,0)+neighbor_array(0,0,-1)+neighbor_array(0,0,1) &
            & +neighbor_array(0,1,0)+neighbor_array(1,0,0) - 6.D0*neighbor_array(0,0,0)

            laplacian = kp(i,j,k)*laplacian/DX/DX

            grid(i,j,k)%copy_ptr%eta = grid(i,j,k)%copy_ptr%eta - lp(i,j,k) * (f0-laplacian) * DT
            grid(i,j,k)%copy_ptr => grid(i,j,k)%copy_ptr%next
          !END ASSOCIATE
        end do
      END DO
    END DO
  END do
  !$OMP END PARALLEL DO

  !call system_clock(count=finish_time)
  !write (*,*) 'The total CPU time =', real(finish_time-start_time)/real(np_rate), ' seconds'

  CALL UpdateSparseData()
  CALL UpdateSumEta()
  !CALL UpdateSharpOri()

END SUBROUTINE PhaseEvolution

SUBROUTINE Output()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  INTEGER :: i, j, k, l, m, ii, jj, kk, nx, ny, nz, gb_site, grain_count, rex_grain_count
  REAL (KIND = DBL):: radius, radius_r, r_frac
  CHARACTER (LEN = 32) :: out_num

  INTEGER, ALLOCATABLE, DIMENSION (:) :: individual_radius

  CALL UpdateSharpOri()

  WRITE (out_num,'(F6.3)') REAL(t)*DT
  OPEN (UNIT = 101, FILE = trim(output_path)//'phi_'//trim(adjustl(out_num))//'.raw', &
      & STATUS = 'replace', ACTION = 'write', FORM = 'binary')

  DO k = 1,z
    DO j = 1,y
      DO i = 1,x
        WRITE (101) sum_eta(i,j,k)
      end do
    END DO
  END DO
  CLOSE (101)

  ALLOCATE (individual_radius(total_q_number))
  individual_radius = 0
  DO k = 1,z
    DO j = 1,y
      DO i = 1,x
        DO l = 1, total_q_number
	        IF (sharp_ori(i,j,k) == l) THEN
  	        individual_radius(l) = individual_radius(l) + 1
  	        EXIT
	        END if
        end do
	    END DO
    END DO
  END DO

  open (unit = 102, file = trim(output_path)//'psd_'//trim(adjustl(out_num))//'.txt', &
        & status = 'replace', action = 'write', form = 'formatted')

  grain_count = 0
  DO l = 1, total_q_number
    IF (individual_radius(l) /= 0) THEN
      grain_count = grain_count + 1
      write (102, '(ES13.6)') (real(individual_radius(l))*DX**3*0.75D0/PI)**0.333333*2.D0
    END IF
  END do
  close(102)

  radius = (REAL(x)*REAL(y)*REAL(z)*DX**3*0.75D0/REAL(grain_count)/PI)**0.333333*2.D0

  IF (t == TINTERVAL) THEN
  	OPEN (UNIT = 107, FILE = TRIM(output_path)//"GrainSize.txt", STATUS = 'replace', ACTION = 'write', FORM = 'formatted')
  END IF

  WRITE (107,'(ES13.6, ES13.6, ES13.6, ES13.6)') REAL(t)*DT, radius

  IF (t == TSTEP) THEN
    CLOSE (107)
  END IF

  DEALLOCATE (individual_radius)

  WRITE (*,*) 'The time = ', REAL(t)*DT, ' finished.'
END SUBROUTINE Output

SUBROUTINE AnisotropyInterface()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  REAL :: s_gb, u_gb
  REAL :: ki, li, yi, mi

  s_gb = SGB_0
  u_gb = UGB_0

  CALL ModelParameter(s_gb, u_gb, ki, li, yi, mi)

  mp = mi
  kp = ki
  lp = li
  yp = yi

END SUBROUTINE AnisotropyInterface

SUBROUTINE ModelParameter(s_gb, u_gb, ki, li, yi, mi)
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  REAL (KIND = DBL), PARAMETER :: p1 =      -5.288D0
  REAL (KIND = DBL), PARAMETER :: p2 =    -0.09364D0
  REAL (KIND = DBL), PARAMETER :: p3 =       9.965D0
  REAL (KIND = DBL), PARAMETER :: p4 =      -8.183D0
  REAL (KIND = DBL), PARAMETER :: p5 =       2.007D0

  REAL (KIND = DBL), PARAMETER :: pp1 =     0.05676D0
  REAL (KIND = DBL), PARAMETER :: pp2 =     -0.2924D0
  REAL (KIND = DBL), PARAMETER :: pp3 =      0.6367D0
  REAL (KIND = DBL), PARAMETER :: pp4 =     -0.7749D0
  REAL (KIND = DBL), PARAMETER :: pp5 =      0.6107D0
  REAL (KIND = DBL), PARAMETER :: pp6 =     -0.4324D0
  REAL (KIND = DBL), PARAMETER :: pp7 =      0.2792D0

  REAL (KIND = DBL) :: f_0, g_0, a_0
  REAL (KIND = DBL) :: a_star, k_star, gamma_star
  REAL (KIND = DBL) :: g, gr, f
  REAL, INTENT (inout) :: ki, li, yi, mi
  REAL, INTENT (in) :: s_gb, u_gb

  f_0 = (2.D0*GAMMA_0-1.D0)/4.D0/(2.D0*GAMMA_0+1.D0)
  g_0 = sqrt(f_0)*4.D0/3.D0
  a_0 = sqrt(f_0)/g_0
  mi = SGB_0/LGB/g_0/sqrt(f_0)

  a_star = a_0
  a_0 = 0.0
  DO WHILE (abs(a_star-a_0) > 1.0D-10)
      a_0 = a_star
      k_star = s_gb*LGB*a_0
      g = s_gb/sqrt(k_star*mi)
      gr = p1*g**8 + p2*g**6 + p3*g**4 + p4*g**2 + p5
      f = pp1*gr**6 + pp2*gr**5 + pp3*gr**4 + pp4*gr**3 + pp5*gr**2 + pp6*gr + pp7
      gamma_star = 1.D0/gr
      a_star = sqrt(f)/g
  END DO

  ki = k_star
  yi = gamma_star
  li = u_gb/LGB/a_star

END SUBROUTINE ModelParameter

SUBROUTINE UpdateSharpOri()
  USE SparseDataStructure
  USE Constant
  USE ExternalSub
  USE IFPORT
  IMPLICIT none

  INTEGER :: i, j, k, ii, jj, kk, nx, ny, nz, gb_site
  REAL (KIND = DBL) :: max_eta

  DO k = 1,z
    DO j = 1,y
      DO i = 1,x
        max_eta = 0.0
        current_ptr => grid(i,j,k)%head_ptr
        DO
          IF (.NOT. ASSOCIATED(current_ptr)) EXIT
          IF (current_ptr%eta > max_eta) THEN
            max_eta = current_ptr%eta
            sharp_ori(i,j,k) = current_ptr%eta_id
          END IF
            current_ptr => current_ptr%next
        END do
      end do
    END DO
  END DO

  !DO j = 1,y
  !  DO i = 1,x
  !    gb_site = 0
  !    gb: DO jj = j-1, j+1
  !      DO ii = i-1, i+1
  !        nx = ii
  !        ny = jj

  !          !periodic boundary
  !        IF (nx == 0) nx = x
  !        IF (nx == x+1) nx = 1
  !        IF (ny == 0) ny = y
  !        IF (ny == y+1) ny = 1

  !        IF (sharp_ori(i,j) /= sharp_ori(nx,ny)) then
  !          gb_site = 1
  !          EXIT gb
  !        END IF
  !      END DO
  !    END DO gb

  !    IF (gb_site == 1) THEN
  !      IF (gb_stat(i,j) == 1) THEN
  !        gb_stat(i,j) = 1
  !        accu_strain(i,j) = accu_strain(i,j) + DSTRAIN*0.5
  !      ELSE
  !        gb_stat(i,j) = 1
  !        accu_strain(i,j) = 0.D0
  !      END IF
  !    ELSE
  !      gb_stat(i,j) = 0
  !      accu_strain(i,j) = 0.D0
  !    END IF
  !  END DO
  !END DO

END SUBROUTINE UpdateSharpOri

