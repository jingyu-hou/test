...
QTBConst::~QTBConst()
{
 ...
 ThermalBoundary::ThermalBoundary(QWidget *parent)
 {
	...
	//��ʼ����������
	 ...
	 m_HBPreBtn = new QPushButton(tr("��һ��"));
	 m_HBNextBtn = new QPushButton(tr("��һ��"));
	 Hlayout->addStretch();
 ...
 void ThermalBoundary::SetInpDataHB(ReadInpResultS ReadInpData)
{
 ...
 void ThermalBoundary::WriteHBToInp()
 {
	...
	for (int i=0; i<TabRowCnt; i++){
		...
		MyCombox *ff = (MyCombox*)(m_TableWiget->cellWidget(i, 0));//�������漯��
		//MyCombox *ff1 = (MyCombox*)(m_TableWiget->cellWidget(i, 5));//������漯��
		...
		//int id1=m_surfFRS.strName.indexOf(str1);//����
		//**********************************************************����
		if (id !=-1){
	...
	DX=m_CCCurveDataListDlg02.size();//����ϵ����ʱ��仯
	for(int II=0;II<DX;II++){
		nodeCurveList=m_CCCurveDataListDlg02.at(II)->m_HIPCurveNode;
		m_CurveList.append(nodeCurveList);
	}

	DX=m_RCCurveDataListDlg02.size();//����ϵ����ʱ��仯
	for(int II=0;II<DX;II++){
